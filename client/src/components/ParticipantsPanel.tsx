...
import { X, Mic, MicOff, Video, VideoOff, Hand, MoreVertical, Shield, UserMinus, MicOff as ForceMuteIcon, Lock, Unlock, MessageSquare, MonitorUp } from 'lucide-react';
          ...
          <div className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-white/20 scrollbar-track-transparent">
            ...
            <div className="p-4 space-y-2">
              ...
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-white text-black flex items-center justify-center relative">
                      ...
                      <AnimatePresence>
                        {openDropdownId === p.peerId && (
                          <motion.div
                            ...
                            className="absolute right-0 mt-2 w-40 bg-black/90 backdrop-blur-xl border border-white/[0.1] rounded-xl shadow-2xl py-1 z-50 overflow-hidden"
                          >
                            <button
                            ...
                            >
                              <ForceMuteIcon className="w-4 h-4 text-slate-400" /> Mute Mic
                            </button>
                            {p.role !== 'co-host' && localRole === 'host' && (
                              <button
                                onClick={() => { onPromotePeer(p.peerId); setOpenDropdownId(null); }}
                                className="w-full text-left px-4 py-2 text-sm text-white hover:bg-white/5 flex items-center gap-2"
                              >
                                <Shield className="w-4 h-4 text-yellow-400" /> Make Co-host
                              </button>
                            )}
                            <button
