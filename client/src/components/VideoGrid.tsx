...
export const VideoGrid: React.FC<VideoGridProps> = ({ children, participantCount }) => {
  ...
  // Mobile-first approach: stack vertically or 2-col on small screens
  let gridClass = '';
  ...
  if (participantCount === 1) {
    ...
    gridClass = 'grid-cols-2 grid-rows-3 md:grid-cols-3 md:grid-rows-2';
  ...
  return (
    <div className="w-full h-full p-1 sm:p-2 md:p-4 bg-surface flex items-center justify-center overflow-hidden">
      <div className={`w-full h-full grid gap-1 sm:gap-2 md:gap-3 ${gridClass}`}>
        {children}
      </div>
    </div>
  );
